...
class CDD(object):
    ...
    def split_classwise(self, dist, nums):
        # split the dis-matrix into class-wise patch that models the pair-wise distance 
    ...
    def compute_paired_dist(self, A, B):
        # compute pairwise L2-distance and return a dis-matrix 
        bs_A = A.size(0)
        bs_T = B.size(0)
        feat_len = A.size(1)
    ...
    def target_only_forward(self, target, nums_T):

        num_classes = len(nums_T)
        ...
        for i in range(self.num_layers):
            ...
            gamma_layers += [self.patch_gamma_estimation(nums_T, nums_T, dist)]
